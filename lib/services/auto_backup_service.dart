...
class AutoBackupService {
  static const String autoBackupName = 'auto';
  ...
  static Future<bool?> checkAndCreateAutoBackup(Game game) async {
    try {
      ...
      if (latestModifyTime == null) {
        LoggingService.instance.info('游戏 ${game.title} 存档目录为空，跳过自动备份');
        return null;
      }
      ...
      if (latestAutoBackup == null ||
        ...
        LoggingService.instance.info('为游戏 ${game.title} 创建了自动备份');
        return true; // 成功创建了备份
      } else {
        LoggingService.instance.info('未检测到存档目录变更，跳过本次自动备份');
        return false; // 跳过了备份
      }
    } catch (e) {
      LoggingService.instance.info('检查自动备份失败: $e');
      return null; // 出错了
    }
  }
  ...
  static Future<DateTime?> _getLatestModifyTime(Directory directory) async {
    ...
    try {
      ...
      LoggingService.instance.info('获取文件修改时间失败: $e');
    }
  ...
  static Future<List<SaveBackup>> _getAutoBackups(String gameId) async {
    try {
      final gameBackups = await SaveBackupService.getGameBackups(gameId);

      // 查找所有自动备份（名称为 "auto" 的备份）
      return gameBackups
          .where((backup) => backup.name == autoBackupName)
          .toList();
    } catch (e) {
      LoggingService.instance.info('获取自动备份列表失败: $e');
      return [];
    }
  }
  ...
  static Future<SaveBackup?> _getLatestAutoBackup(String gameId) async {
    try {
      ...
      LoggingService.instance.info('获取最新自动备份失败: $e');
      return null;
    }
  }
  ...
  static Future<void> _createAutoBackup(Game game) async {
    try {
      ...
      if (backup != null) {
        LoggingService.instance.info('自动备份创建成功: ${backup.id}');

        // 检查是否需要删除旧备份
        await _cleanupOldAutoBackups(game.id, maxBackupCount);
      } else {
        LoggingService.instance.info('自动备份创建失败');
      }
    } catch (e) {
      LoggingService.instance.info('创建自动备份失败: $e');
    }
  }
  ...
  static Future<void> _cleanupOldAutoBackups(
    ...
    try {
      await ResticService.cleanSnapshot(tags: ['game:$gameId', 'name:auto'], keep: maxCount);

      final cloudConfig = await CloudSyncConfigService.getCloudSyncConfig();
      if(cloudConfig != null) {
        await ResticService.cleanSnapshot(tags: ['game:$gameId', 'name:auto'], useRemote: true, cloudConfig: cloudConfig, keep: maxCount);
      }

      LoggingService.instance.info('清理完成，保留了 $maxCount 个最新的自动备份');
    } catch (e, s) {
      LoggingService.instance.logError('清理旧自动备份失败', e, s);
    }
  }

  // 检查备份是否为自动备份
  static bool isAutoBackup(SaveBackup backup) {
    return backup.name == autoBackupName;
  }
  ...
  static Future<void> deleteAutoBackups(String gameId) async {
    try {
      ...
      for (final backup in autoBackups) {
        ...
        LoggingService.instance.info('删除自动备份: ${backup.id}');
      }
    } catch (e) {
      LoggingService.instance.info('删除自动备份失败: $e');
    }
  }
  ...
  static Future<BackupCheckResult> checkAutoBackupBeforeLaunch(
    ...
    try {
      ...
      if (!await saveDataDir.exists()) {
        LoggingService.instance.info('存档目录不存在，建议应用自动备份: ${game.title}');
        return BackupCheckResult(
          shouldApply: true,
      ...
      // 如果存档目录为空，建议应用自动备份
      if (latestModifyTime == null) {
        LoggingService.instance.info('存档目录为空，建议应用自动备份: ${game.title}');
        return BackupCheckResult(
          shouldApply: true,
          autoBackupTime: autoBackup.createdAt,
          saveDataTime: null,
      ...
      LoggingService.instance.info('检查启动前备份应用失败: $e');
      return BackupCheckResult(
        shouldApply: false,
  ...
  static Future<bool> applyAutoBackup(Game game) async {
    try {
      ...
      if (success) {
        LoggingService.instance.info('自动备份应用成功: ${game.title}');
      } else {
        LoggingService.instance.info('自动备份应用失败: ${game.title}');
      }
      return success;
    } catch (e) {
      LoggingService.instance.info('应用自动备份失败: $e');
      return false;
    }
  }
}
